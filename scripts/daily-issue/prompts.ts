...
- daily_case: EXACTLY 1 case card per day. A tight 3-section MARKETING teardown (≈ 1.5-minute read). Keep the section visible every day.
  ...
  Marketer-relevance check (MANDATORY): the case subject must fit ONE of these profiles:
    (a) AI product / platform / company operating in marketing, growth, brand, distribution, creator economy, content, adtech, martech, or commerce (e.g. Lovable, Canva AI, Pencil, Cursor for marketing copy, HubSpot, Shopify AI, creator-tool startups, adtech vendors).
    (b) AI news that reveals a transferable marketing / growth / brand / distribution tactic even when the subject is a non-marketing AI product (e.g. a lab's launch playbook, an AI company's go-to-market pattern worth stealing).
    (c) An incident or shift where AI's impact on marketing practice IS the central lesson (SEO CTR drop from AI Overviews, brand safety failure, attribution model change, creator vs. AI-content discourse).
  REJECT candidates that are purely about AI infrastructure, scientific research, hardware, geopolitics, benchmarks, funding rounds with no marketing angle, or company news with no marketer takeaway. When rejected, emit 0 cards — the section falls back to the empty-window placeholder; one bad case is worse than no case.

  Editorial bar (SOFT on content depth): if the marketer-relevant candidate has at least 1 concrete anchor (number / named product / positioning / timeline / quote) and one strategic angle, ship it — narrower takeaways are better than blank. Only skip if the candidate is literally empty / pure speculation / single-sentence news with no content at all.
